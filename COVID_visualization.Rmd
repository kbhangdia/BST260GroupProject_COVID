...
#Merging map data with cleand_df
Clean_df_joined <- left_join(clean_df_updated, Counties_df, by = "county")
...
Clean_df_joined %>%
  ...
  geom_polygon(aes(fill = cases), color = "grey", size = .1) +
...
Clean_df_joined %>%
  ...
  geom_polygon(aes(fill = cases_per_1000ppl), color = "grey", size = .1) +
...
clean_df_updated %>% 
   ...
   labs(title = "Covid-19 cases versus Population Density in the U.S in 30-Nov-2020",caption = "Data source: XXXXXXX." ) + 
...
clean_df_updated %>% 
   ...
   ylab("Case Rate (per 1,000)") +
   labs(title = "Covid-19 cases versus average household size in the U.S in 30-Nov-2020",caption = "Data source: XXXXXXX." ) + 
  scale_y_continuous(limits = c(0, 1000)) +
  theme_bw()
...
clean_df_updated %>% 
   ...
   ylab("Case Rate (per 1,000)") +
   labs(title = "Covid-19 cases versus Homeworkers Density in the U.S in 30-Nov-2020",caption = "Data source: XXXXXXX." ) + 
  scale_y_continuous(limits = c(0, 1000)) +
  theme_bw()
...
clean_df_updated %>% 
   ...
   ylab("Case Rate (per 1,000)") +
   labs(title = "Covid-19 cases versus poverty level in the U.S in 30-Nov-2020",caption = "Data source: XXXXXXX." ) + 
  scale_y_continuous(limits = c(0, 1000)) +
  theme_bw()
...
clean_df_updated_ %>% 
   ...
   ylab("Case Rate (per 1,000)") +
   labs(title = "Covid-19 cases versus population over 65 in the U.S in 30-Nov-2020",caption = "Data source: XXXXXXX." ) + 
  scale_y_continuous(limits = c(0, 1000)) +
  theme_bw()

```
```{r}
#Race and COVID Case Rates --> pending.. ask how to define the varaible "race"
